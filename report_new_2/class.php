...
class Report extends CBitrixComponent {
    ...
    public function executeComponent() {
        ...
        $arResult['DATA_TITLES'] = [
            'call_outgoing'            => 'Звонки исходящие',
            'call_outgoing_2min'       => 'Звонки исходящие (Более 2х мин)',
            'email_sent'               => 'E-mail отправленные',
            'conversion_company'       => 'Конверсия Компаний из ПКБ в АКБ',
            'calc_deal'                => 'Открыто Сделок в расчет ПТО',
            'deal_failed'              => 'Забраковано Сделок',
            'product_testing'          => 'Открыто Испытаний',
            'product_testing_complete' => 'Завершено Испытаний',
            'production'               => 'Запущен БП Производства Заказа',
            'production_summ'          => 'Cумма запущенных в пр-во заказов (руб без НДС)',
            'orders_shipped'           => 'Отгружено Заказов',
            'orders_shipped_summ'      => 'Сумма отгруженных заказов (руб без НДС)',
            'prihod_ds'                => 'Приход ДС (руб без НДС)',
            'prihod_ds_whithNDS'       => 'Приход ДС (руб с НДС)',
			'summ_for_deal'       => 'Ожидаемые поступления без НДС',    //добавили title нового столбца
            'planned_shipments'       => 'Планируемые отгрузки',
        ];
    ...
    public static function fReport_hlp_get_DetailСallAndEmail(&$result, $params) {
        ...
        while ($ar = $res->Fetch()) {
            ...
            */
            ...
            if ($ar['act_TYPE_ID'] == 2) {
                // Call
                $arResultKeyTMP[] = 'call_outgoing';

                if ($duration > (60 * 2)) {
                    $arResultKeyTMP[] = 'call_outgoing_2min';
                }
            } else {
                // Email
                $arResultKeyTMP[] = 'email_sent';
            }
    ...
    public static function fReport_hlp_get_product_testing_hlp(&$result, $params) {
        ...
        if ($resultKey == "conversion_company") {
        ...
        while ($ar = $res->fetch()) {
            ...
            */
            ...
            if ( $resultKey == 'production_summ' || $resultKey == 'summ_for_deal' || $resultKey == 'planned_shipments') {
            ...
            } else if ($resultKey == 'conversion_company') {
    ...
    public static function fReport_hlp_get_product_testing_complete(&$result, $params) {
        ...
        if (0) {
            ...
            self::fReport_hlp_queryCommon([
                'result'    => &$result,
                'resultKey' => 'product_testing_complete',
                'query'     => $query,
                'fieldDate' => 'MODIFIED',
                'dateFrom'  => $params['dateFrom'],
                'dateTo'    => $params['dateTo'],
    ...
    public static function fReport_hlp_get_orders_shipped(&$result, $params) {
        ...
        if (is_null($params['dateTo'])) {
            $t_arFilter = Array("TITLE" => "%произвести заказ%","RESPONSIBLE_ID" => "22",'>=CLOSED_DATE'=>$params['dateFrom']->format('d.m.Y')); 
                ...
                $t_arFilter = Array("TITLE" => "%произвести заказ%",
                "RESPONSIBLE_ID" => "22",
...
echo "<script>console.log('params'); console.log(".json_encode($params).")</script>";
        $resultKey    = 'summ_for_deal';
    ...
    public static function fReport_hlp_get_total(&$result) {
        ...
        $total['items']['prihod_ds']['price'] = 0;
        $total['items']['prihod_ds_whithNDS']['price'] = 0;
        $total['items']['production_summ']['price'] = 0;
        $total['items']['summ_for_deal']['price'] = 0;
        $total['items']['planned_shipments']['price'] = 0;

        foreach ($result as $value) {
            foreach ($value['items'] as $resultKey => $value_2) {
                $total['items'][ $resultKey ]['count'] += $value_2['count'];
                if ($resultKey == 'prihod_ds') {
                    $total['items'][ $resultKey ]['price'] += $value_2['price'];
                }
                if ($resultKey == 'prihod_ds_whithNDS') {
                    $total['items'][ $resultKey ]['price'] += $value_2['price'];
                }
                if ($resultKey == 'production_summ') {
                    $total['items'][ $resultKey ]['price'] += $value_2['price'];
                }
                if ($resultKey == 'orders_shipped_summ') {
                    $total['items'][ $resultKey ]['price'] += $value_2['price'];
                }
                if ($resultKey == 'summ_for_deal') {
                    $total['items'][ $resultKey ]['price'] += $value_2['price'];
                }
                if ($resultKey == 'planned_shipments') {
                    $total['items'][ $resultKey ]['price'] += $value_2['price'];
                }
            }
        } //
